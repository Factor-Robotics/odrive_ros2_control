...
CallbackReturn ODriveHardwareInterface::on_init(const hardware_interface::HardwareInfo& info)
{
  ...
  for (size_t i = 0; i < info_.joints.size(); i++)
  {
    ...
    CHECK_TS(odrive->read(serial_numbers_[1][i], AXIS__MOTOR__CONFIG__TORQUE_CONSTANT + per_axis_offset * axes_[i],
                          torque_constant));
    torque_constants_.emplace_back(torque_constant);

    if (enable_watchdogs_[i])
    {
      CHECK_TS(odrive->write(serial_numbers_[1][i], AXIS__CONFIG__WATCHDOG_TIMEOUT + per_axis_offset * axes_[i],
                             std::stof(info_.joints[i].parameters.at("watchdog_timeout"))));
    }
    CHECK_TS(odrive->write(serial_numbers_[1][i], AXIS__CONFIG__ENABLE_WATCHDOG + per_axis_offset * axes_[i],
...
CallbackReturn ODriveHardwareInterface::on_activate(const rclcpp_lifecycle::State& previous_state)
{
  for (size_t i = 0; i < info_.joints.size(); i++)
  {
    if (enable_watchdogs_[i])
    {
      CHECK_TS(odrive->call(serial_numbers_[1][i], AXIS__WATCHDOG_FEED + per_axis_offset * axes_[i]));
    }
...
CallbackReturn ODriveHardwareInterface::on_deactivate(const rclcpp_lifecycle::State& previous_state)
{
  ...
  for (size_t i = 0; i < info_.joints.size(); i++)
  {
    CHECK_TS(odrive->write(serial_numbers_[1][i], AXIS__REQUESTED_STATE + per_axis_offset * axes_[i], requested_state));
  }
    ...
    if (control_level_[i] != new_modes[i])
    {
      ...
      switch (new_modes[i])
      {
        case integration_level_t::UNDEFINED:
          ...
          CHECK_RW(odrive->write(serial_numbers_[1][i], AXIS__REQUESTED_STATE + per_axis_offset * axes_[i],
                                 requested_state));
          break;
        ...
        case integration_level_t::EFFORT:
          ...
          CHECK_RW(odrive->write(serial_numbers_[1][i],
        ...
        case integration_level_t::VELOCITY:
          ...
          CHECK_RW(odrive->write(serial_numbers_[1][i], AXIS__CONTROLLER__INPUT_VEL + per_axis_offset * axes_[i],
                                 input_vel));
          input_torque = hw_commands_efforts_[i];
          CHECK_RW(odrive->write(serial_numbers_[1][i], AXIS__CONTROLLER__INPUT_TORQUE + per_axis_offset * axes_[i],
                                 input_torque));
        ...
        case integration_level_t::POSITION:
          ...
          CHECK_RW(odrive->write(serial_numbers_[1][i], AXIS__CONTROLLER__INPUT_POS + per_axis_offset * axes_[i],
                                 input_pos));
          input_vel = hw_commands_velocities_[i] / 2 / M_PI;
          CHECK_RW(odrive->write(serial_numbers_[1][i], AXIS__CONTROLLER__INPUT_VEL + per_axis_offset * axes_[i],
                                 input_vel));
...
return_type ODriveHardwareInterface::read(const rclcpp::Time& time, const rclcpp::Duration& period)
{
  ...
  for (size_t i = 0; i < info_.joints.size(); i++)
  {
    ...
    CHECK_RW(odrive->read(serial_numbers_[1][i], AXIS__MOTOR__FET_THERMISTOR__TEMPERATURE + per_axis_offset * axes_[i],
                          fet_temperature));
    hw_fet_temperatures_[i] = fet_temperature;

    CHECK_RW(odrive->read(serial_numbers_[1][i],
                          AXIS__MOTOR__MOTOR_THERMISTOR__TEMPERATURE + per_axis_offset * axes_[i], motor_temperature));
