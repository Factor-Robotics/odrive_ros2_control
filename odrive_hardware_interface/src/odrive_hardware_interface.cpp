...
return_type ODriveHardwareInterface::start()
{
  for (size_t i = 0; i < info_.joints.size(); i++)
  {
    ...
    CHECK(odrive->call(odrive->odrive_handles_[i], CLEAR_ERRORS));
  }
...
return_type ODriveHardwareInterface::read()
{
  for (size_t i = 0; i < info_.joints.size(); i++)
  {
    ...
    CHECK(odrive->read(odrive->odrive_handles_[i],
                       AXIS__MOTOR__FET_THERMISTOR__TEMPERATURE + per_axis_offset * axis_[i], fet_temperature));
    hw_fet_temperatures_[i] = fet_temperature;

    CHECK(odrive->read(odrive->odrive_handles_[i],
                       AXIS__MOTOR__MOTOR_THERMISTOR__TEMPERATURE + per_axis_offset * axis_[i], motor_temperature));
