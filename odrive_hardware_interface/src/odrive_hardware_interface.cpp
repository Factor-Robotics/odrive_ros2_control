...
CallbackReturn ODriveHardwareInterface::on_activate(const rclcpp_lifecycle::State& previous_state)
{
  for (size_t i = 0; i < info_.joints.size(); i++)
  {
    ...
    CHECK_TS(odrive->call(serial_numbers_[1][i], AXIS__CLEAR_ERRORS + per_axis_offset * axes_[i]));
  }
...
return_type ODriveHardwareInterface::read(const rclcpp::Time& time, const rclcpp::Duration& period)
{
  ...
  for (size_t i = 0; i < info_.joints.size(); i++)
  {
    ...
    CHECK_RW(odrive->read(serial_numbers_[1][i], AXIS__MOTOR_THERMISTOR__TEMPERATURE + per_axis_offset * axes_[i],
                          motor_temperature));
